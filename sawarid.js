// Firebase Configuration
      ...
      const playlists = [
        ...
        { url: 'https://raw.githubusercontent.com/buzzinguaecn/Pizza/main/LiveEventSports.m3u', name: "Live Sports", displayName: "Live Sports" },
      ...
      // ওএসডি এবং মাউস কার্সার ২ সেকেন্ডের নিষ্ক্রিয়তায় হাইড করার লজিক
      ...
      // Shaka Native Video Elements Events Sync (মিউট, আনমিউট এবং পোস্টার সিঙ্ক)
      ...
      async function cleanUpPlayers() {
        ...
        shakaWatermark.classList.add('hidden'); // Shaka ওয়াটারমার্ক রিসেট করা
      }
      ...
      function renderChannelsGrid(targetChannels = parsedChannels) {
        ...
        targetChannels.forEach((ch, idx) => {
          ...
          card.className = `p-2 rounded-lg border-2 border-black flex flex-col items-center justify-center text-center w-full transition-all outline-none outline-2 outline-black ripple-btn group channel-item ${
            isActive 
              ? 'border-white shadow-[0_0_12px_rgba(255,255,255,0.85),_0_0_24px_rgba(23,72,232,0.65)] bg-[#0656bf]' 
          ...
          const isLongName = ch.name.length > 13;
        ...
        if (isDASH) {
          ...
          try {
            ...
            player.configure({
              ...
              abr: {
                enabled: true, 
                defaultBandwidthEstimate: 1800000, 
                switchInterval: 2 
              },
              drm: {
                delayLicenseRequestUntilPlayed: false // লাইসেন্স লোড হওয়ার কারণে ভিডিও পজ হওয়ার সমাধান
              }
            });
            ...
            // শাকা প্লেয়ার আনমিউট অবস্থায় দ্রুত অটো-প্লে নিশ্চিতকরণ
            videoElement.play().then(() => {
              isPlaying = true;
              videoPoster.classList.add('opacity-0');
              bufOverlay.classList.add('hidden');
            }).catch(e => {
              console.warn("Unmuted autoplay restricted. Retrying with mute fallback...", e);
      ...
      function seekVideo(event) {
        if (videoElement.duration && isFinite(videoElement.duration)) {
          ...
          showToast("Seeking is not supported on live transmissions.");
        }
      }
      ...
      window.addEventListener('beforeinstallprompt', (e) => {
        // ক্রোম ডিফল্ট প্রম্পট আটকে সেভ করে রাখা
        e.preventDefault();
        deferredPrompt = e;
        // ইনস্টল বাটন স্ক্রিনে শো করা
      ...
      window.addEventListener('appinstalled', () => {
        deferredPrompt = null;
        document.getElementById('pwaInstallBtn').classList.add('hidden');
        showToast('SAWARID App successfully installed on your device!');
      });

      // PWA সার্ভিস ওয়ার্কার রেজিস্ট্রি
      ...
      async function initApp() {
        ...
        // Setup Interface Click Handlers
